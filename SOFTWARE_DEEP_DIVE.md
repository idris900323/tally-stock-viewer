...
- `templates/index.html`
  ...
  - the `More` popover (`#moreMenu`, `toggleMoreMenu()`/`openMoreMenu()`/`closeMoreMenu()`) holds six items behind one trigger: `Full Refresh`, `Manage Accounts`, `System` (a visual divider), `Category Settings` (section 22's admin-editable category list — a visual divider), then `Needs Category` and `Needs Image Matching` (section 23) — closes on item click, outside click, or Escape; every item runs through `handleMoreMenuAction()` so the menu can never linger open after a selection
...
- The BURNED badge on a shared image (`_draw_category_badge()`, section 13) also reads the abbreviation now, not the full category name — the same field, so the two stay visually consistent, and an admin fixing an awkward auto-generated abbreviation in the System panel fixes both the ribbon and the share badge in one edit. Renaming, merging, or overriding a category's abbreviation forces a regeneration of every affected item's cached badge (`_regenerate_badges_for_stock_items()`), since the passive mtime-based staleness check has no way to notice a same-named category's abbreviation text changed underneath it.
