...
## 29. Popup, important banner, and customer reports

**Popup and Important Banner (two unrelated features).** Each has its own single-row table (`popup_notice`, `banner_notice`), endpoints and JS; they share nothing. *Popup*: optional text and/or image (at least one), `version` bumped on every publish, shown to customers via `/api/popup`, closed only with its X icon, dismissal stored in `localStorage` (`popupDismissedVersion`) against that version. *Banner*: required text, `/api/banner`, a scrolling marquee with no close control that slides away once the customer scrolls past 80px and returns when they are back within 10px of the top (the sticky toolbar's `top` follows `--notice-banner-h`); only publishing new text or clearing it changes it. Admins manage both from More > Popup & Banner (two separate sections). Neither endpoint returns anything to admin sessions. Popup images live in `data/notice_images/`. An older merged two-slot table (`notice_slots`) is migrated once and dropped at startup.

**Report button and bell.** Customers get one button below a car's design list (shown only if that car has an incomplete design) that POSTs `/api/report_item` with just the car. The server works out which designs lack an image or category, stores a summary, and a partial unique index on unresolved rows (`customer_reports`) keeps it to one open report per car. Admins see the open count as a badge on a bell icon beside Logout (index.html header, polled every 30s from `/api/customer_reports`); clicking it opens a dropdown listing each report with Mark Resolved (`/admin/customer_reports/<id>/resolve`). Reports are not part of the Work Queue.
